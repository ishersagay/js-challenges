...
export const getIsValidOnNameTag = () => {
  const isValid = (nameTagOption.length <= 8 && nameTagOption.charAt(0) === nameTagOption.charAt(0).toUpperCase())
    return isValid;
};
...
export const convertStringToNumber = () => {
  const num = parseFloat(stringToConvert);
  return num;
};
...
export const getHasUppercaseLetters = () => {
   const Upper = (/[A-Z]/).test(stringWithUppercaseLetters);
    return Upper;
};
